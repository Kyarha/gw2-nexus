...
**Direction chosen (2026-08-12):** Design mockups use a **CSS approximation** of the native look (not shipped, so no reuse question). In the shipped addon, **prefer reusing the real GW2 UI texture at runtime by asset ID** (Blish-style — loaded on the player's machine, never bundled/redistributed) *if* it looks better and the Nexus C++ API exposes it; **original art in the GW2 style is the fallback**. Only open sub-item is the feasibility spike below.

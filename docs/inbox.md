...
- [2026-08-13] Legendary crafting + Trading Post brainstorm captured in [docs/research/legendary-and-trading-post-ideas.md](research/legendary-and-trading-post-ideas.md) — two player painpoints ("where do I begin on a legendary" + "TP price alarms / per-character wishlist"), neither built today; both hinge on the unbuilt `shared/` GW2 API client. Sequenced idea: API client → Legendary *next-step* tracker → TP watch/wishlist. Triage: promote to specs (API-client foundation first) after the 003 MVP.
